...
import com.ulp.features.comments.service.LessonCommentsService;
...
class LessonCommentsApiControllerTest {
    ...
    private static final String LECTURER_EMAIL = "lecturer@ulp.edu.vn"; // owns the class
    private static final String ADMIN_EMAIL = "admin@ulp.edu.vn";    // ADMIN, not enrolled
    ...
    // ── Moderation: hide / unhide (ULP-11.7) ──────────────────────────
    ...
    void lecturer_unhide_returns_ok() throws Exception {
        ...
        service.hide(lesson.getId(), root.id(), lecturer.getId(), com.ulp.security.Role.LECTURER);
    ...
    // ── Bulk moderation: bulk/hide, bulk/unhide (ULP-11.7) ─────────────
    ...
    void lecturer_bulk_unhide_returns_counts() throws Exception {
        CommentRow a = service.create(lesson.getId(), author.getId(), "A", null);
        CommentRow b = service.create(lesson.getId(), author.getId(), "B", null);
        service.hide(lesson.getId(), a.id(), lecturer.getId(), com.ulp.security.Role.LECTURER);
        service.hide(lesson.getId(), b.id(), lecturer.getId(), com.ulp.security.Role.LECTURER);
